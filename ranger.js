...
var loop_speed = 500 / character.frequency; // this is in milliseconds.
var low_pot_amount = 30; // when you have less than this number, go restock potions.
var give_whitelist = ["hpbelt","hpamulet","ringsj","seashell","gem0","cscroll0","scroll0"];
var sell_whitelist = ["hpbelt","hpamulet"]

map_key("4","snippet","send_cm('joyous18','send')");

if(!get_player("joyous18")) {
	start_character("joyous18",3);
	//
}
//show_json(character.x+","+character.y)
...
function handle_potions(){
  ...
  if(can_use('use_hp') || can_use('use_mp')){ // if your cooldown for using potions has ended,
    if(character.hp < 200){
      ...
      if(character.mp < 300){
      if (character.mp <= character.max_mp - 600) use('use_mp'); // if your mana is less than what you need to attack, use a potion.
    } else {
      if (character.mp <= character.max_mp - 600) use('use_mp');
    }
  }
...
function handle_farming(){
  ...
  var monsters = entities.filter(m => !m.rip && m.mtype == farm_list && !m.target);
    ...
    if(monsters.length < 1){ // if there are no desired monsters near us,
        ...
        if(!smart.moving) smart_move(-80.8473269784452,1839.9999999); // this smart moves to the monster types you want to farm.
    } else {
        var monster = get_nearest_monster({type:farm_list[0]});
		// since there are desired monsters near, get the closest one to attack.
    }
   
    if(monster){ // if we have a desired monster available,
        if(in_attack_range(monster)){ // and we are close enough to attack it,
			
            if(can_use("3shot") && character.mp > 323) {
			   use_skill("3shot")
			}
			else if(can_attack(monster)) {
				attack(monster); // if we can attack it, attack it.
			}
             // we return since we don't need to move to it.
        } else {
			
            if(!is_moving(character)) // if we are not moving already,
            move( // move half the distance to the monster.
                character.x+(monster.x-character.x)/5,
                character.y+(monster.y-character.y)/5
            );
        }
  }
