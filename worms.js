...
var farm_list = ["stoneworm"]; // the monster type you want to farm.
...
var give_whitelist = ["hpbelt","hpamulet","strbelt","stramulet","intbelt","intamulet","ringsj","seashell","gem0","cscroll0","scroll0"];

if(!get_player("joyous18")) {
	//start_character("joyous18",3);
	//send_party_invite(get_player("joyous18"))
}
if(!get_player("dfsdfa")) {
	//start_character("dfsdfa",4);
	//send_party_invite(get_player("joyous18"))
}
...
function handle_farming(){
  ...
  var monsters = entities.filter(m => !m.rip && m.mtype == farm_list && !m.target);
    ...
    if(monsters.length < 1){ // if there are no desired monsters near us,
        ...
        if(!smart.moving) {
			smart_move(farm_list[0]);
			//smart_move(392.80039367753966,17.154678444726564);
		}// this smart moves to the monster types you want to farm.
	}else {
    ...
    if(monster){ // if we have a desired monster available,
        if(in_attack_range(monster)){ // and we are close enough to attack it,
			...
			if(Math.sqrt(//if it too close
				Math.pow(get_x(monster) - character.x,2) +
				Math.pow(get_y(monster) - character.y,2)) < 
			character.range/1.4) {
				if (!smart.moving) {//run to fallback
					xmove(-86.93565639881538,6.56743956071524);
				}
			} else {
				stop()
			}
			...
			if(!smart.moving) {
				xmove( // move  to the monster.
					character.x+(monster.x-character.x)/7,
					character.y+(monster.y-character.y)/7
				);
			}
        }
  	} 
