...
function handle_inventory(){
  ...
  if(visit_town){ // if you're going to restock potions,
    ...
    var farm_spot = {x: cx, y: cy+100, map: cm};
...
function handle_farming(){
  ...
  // and make sure nobody else has already attacked them.
    ...
    var monster = get_nearest_monster({min_xp:100,max_att:200,path_check:true}); // since there are desired monsters near, get the closest one to attack.
    if(!monster) {
		monster = get_nearest_monster({min_xp:100,max_att:200,path_check:true});
	}
    ...
    if(monster){ // if we have a desired monster available,
        if(in_attack_range(monster)){ // and we are close enough to attack it,
            ...
            move( // move half the distance to the monster.
                character.x+(monster.x-character.x)/20,
                character.y+(monster.y-character.y)/20
            );
        }
  }
